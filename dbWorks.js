...
const dbWorks = {
  ...
  getTotalUserCount: (args) => {
    const result = database.users.length;
    return result;
  },
  // try {
  ...
  logIn: (args) => {
    const result = database.users.find((u) => u.email == args.input.email);
    if (!result) return new Error("no such user");
    if (result.password !== args.input.password) {
